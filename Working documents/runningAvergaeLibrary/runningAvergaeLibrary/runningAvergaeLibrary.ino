...
int MpointAvg = 2; // avg of two points
...
const int breathingData=A0; // breathing data
...
{ 
  ...
  initialTime = millis();
   ...
   pinMode(9, OUTPUT); // vref settings
   analogWrite(9,90.8); // Vref settings
} 
...
{ 
  ...
  rawData = analogRead(breathingData); 

  // add to the array 

  myRA.addValue(analogRead(breathingData)); 
